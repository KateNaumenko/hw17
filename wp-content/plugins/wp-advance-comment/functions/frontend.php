		...
		foreach ( $extra_fields as $key => $value ) {
			...
			switch ( $value['type'] ) {
				...
				case 'reCaptcha':
					...
					if( !cfb_is_plugin_active( 'wp-advanced-comment-pro-addon/index.php' ) || is_admin() ){
						continue;
					}
...
function cfb_add_comment_fields_logged_in() {
    ...
    foreach ( $fields as $name => $field ) {
    	...
    	switch ( $name ) {
    		...
    		case 'comment_field':
    			$comment_field = '<p class="comment-form-comment"><label for="comment">' .esc_html__( 'Comment' , 'wpad' ). '<span class="required"> *</span></label><textarea id="comment" name="comment" cols="45" rows="8" aria-required="true" required="required"></textarea></p>';
		...
		if( !empty( $value['required'] ) && $value['required'] == 'yes' ){
			...
			switch ( $value['type'] ) {
				...
				case 'file_upload':
					...
					if( empty( $_FILES[$name]['name'][0] ) ){
						$message = esc_html__( 'Please add some files' , 'wpad' );
				...
				case 'user_image':
					...
					if( count( $_FILES[$name]['name'] ) > 1 ){
						$message = esc_html__( 'Please upload only one image' , 'wpad' );
						cfb_get_error_message_files( $message , $value );
					}

					// Check empty image
					if( empty( $_FILES[$name]['name'][0] ) ){
						$message = esc_html__( 'Please add your image' , 'wpad' );
						cfb_get_error_message_files( $message , $value );
					}
				...
				case 'google_maps':
					...
					if( count( array_filter( $map_details ) ) < 3 ){
						$message = esc_html__( 'Please add the address properly' , 'wpad' );
						cfb_get_error_message_files( $message , $value );
					}
			...
			switch ( $value['type'] ) {
				...
				case 'reCaptcha':
					...
					if ( !$resp->isSuccess() ){
						$message = esc_html__( 'Verification failed. Please try again' , 'wpad' );
						cfb_get_error_message_files( $message , $value );
					} 
...
function cfb_get_choices_to_array( $value ){
	...
	if( !empty( $value['type'] ) && $value['type'] == 'taxonomy' && cfb_is_plugin_active( 'wp-advanced-comment-pro-addon/index.php' ) ){
