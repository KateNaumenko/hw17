...
function cfb_rename_metaboxes(){
	...
	$wp_meta_boxes['comment_form_builder']['side']['core']['submitdiv']['title'] = esc_html__( 'Form Elements', 'wpad' );
}
...
function cfb_screen_editor_callback( $post ) { ?>
	...
	<a href="javascript:void(0)" class="button toggle_all"><?php esc_html_e( 'Toggle All' , 'wpad' ); ?></a>
	...
	foreach( $data as $key => $value ){
		...
		switch ( $value['type'] ) {
            ...
            case 'taxonomy':
                ...
                if( is_plugin_active( 'wp-advanced-comment-pro-addon/index.php' ) )
...
function cfb_get_input_show_to_admin( $count , $value ){ ?>
    ...
    <div class="wpuf-form-rows required-field">
        ...
        <div class="wpuf-form-sub-fields">
            ...
            <p class="description mt-5">Submitted data will be displayed to admin only</p>
        </div>
...
function cfb_get_backend_custom_fields_btn(){
	...
	$data = array(
		array(
			...
			'title' => esc_html__( 'Custom Fields', 'wpad' ),
			'before_title' => '<h4>',
			'after_title' => '</h4>',
			'fields' => array(
				'text' => array(),
		...
		array(
			...
			'title' => esc_html__( 'Custom Taxonomies', 'wpad' ),
			'before_title' => '<h4>',
			'after_title' => '</h4>',
			'fields' => array(
		...
		array(
			'class' => 'cfb_custom_fields_wrapper mt-10 mb-20',
			'title' => esc_html__( 'PRO Custom Fields', 'wpad' ),
			'before_title' => '<h4>',
			'after_title' => '</h4>',
			'fields' => array(
				'date' => array(
...
function cfb_about_plugin( $post ) { ?>
    ...
    <p>Please report any bug on the <a target="_blank" href="https://github.com/ravishakya/wp-advanced-comment/issues/new?title=Bug%20Report:%20%3Cshort%20description%3E&labels=bug">GitHub Page</a> rather than on the WordPress Support page.</p>

    <?php
}

function cfb_pro_version_notice(){ ?>

    <a target="_blank" href="https://codecanyon.net/item/wp-advanced-comment-pro-/14557575?ref=ravishakya">Click Here</a> to view the PRO Version.
